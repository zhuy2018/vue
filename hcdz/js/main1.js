...
function mainTop(){
  ...
  $('.timetxt dl dd label').attr('tbar', 'true').on('click', function (){
    if( $(this).attr('tbar') =='true' ){
      $(this).addClass('label')
      $(this).attr('tbar', 'false');
    }else{
  ...
  var iTbar=true;
  $('.timetxt dl dt').on('click', function (){
    if( iTbar ){
      $('.timetxt dl dd label').removeClass('label').attr('tbar', 'false');
      iTbar=false;
    }else{
      $('.timetxt dl dd label').addClass('label').attr('tbar', 'true');
      iTbar=true;
    }
  })
}
...
function showHighcharts(){
  ...
  chart = new Highcharts.Chart({
    ...
    series: [{
      name: '东广场',
      data: [700, 690, 905, 14005, 18004, 2105, 2502, 26005, 2303, 1803, 1309, 906]
    },{
      name: '西广场',
      data: [7100, 6190, 905, 14005, 18004, 2105, 2502, 26005, 2303, 1803, 1309, 906]
    }]
  });
}
...
function iShow(){
  ...
  $(traffic).each(function (i,obj){
    if((obj.data.substring(8,10)+obj.data.substr(obj.data.length-3)) <= _time){
      ...
      iHtml+='<td> '+ toThousands($.trim(obj.dsquare)) +' </td>'
      iHtml+='<td> '+ toThousands($.trim(obj.xsquare)) +' </td>'
      iHtml+='<td> '+ toThousands($.trim(obj.parking)) +' </td>'
      iHtml+='</tr>'
    }
  })
  $('.tableDiv table tbody').html(iHtml)
}
