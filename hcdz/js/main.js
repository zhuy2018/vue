...
var leged = false;
var titles;     //标题
var result1=[],result2=[],result3=[];  //数据筛选
var size=[];    //选择数据
var arrVal,strVal;
...
function iShow(arr){
  ...
  $(traffic).each(function (i,obj){
    if((obj.data.substring(8,10)+obj.data.substr(obj.data.length-3)) <= _time){
      ...
      iHtml+='<td> '+ toThousands($.trim(obj.ons)) +' </td>'
      iHtml+='<td> '+ toThousands($.trim(obj.one)) +' </td>'
      iHtml+='<td> '+ toThousands($.trim(obj.two)) +' </td>'
      iHtml+='</tr>';
      result1.push( [Number( (obj.data).substring(8,10)+'.'+obj.data.substr(obj.data.length-3) ), Number(obj.ons)] )
      result2.push( [Number( (obj.data).substring(8,10)+'.'+obj.data.substr(obj.data.length-3) ), Number(obj.one)] )
      result3.push( [Number( (obj.data).substring(8,10)+'.'+obj.data.substr(obj.data.length-3) ), Number(obj.two)] )
    }
  })
